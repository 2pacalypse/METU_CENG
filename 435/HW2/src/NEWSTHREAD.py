...
class RDTSender():
    def __init__(self, sock, ip, port, data, start, end):
        ...
        self.nextseqnumber = 0 # Next available sequence number
        self.acks = [False] * self.N # To see what has been acked in the window
        ...
        self.times = [0] *self.N # Timers of the packets in the window
        ...
        while(self.flag):
            ...
            for i in range(len(self.times)):
                if(self.times[i] < t and self.times[i] > 0):
                   ...
                   packet = self.make_datapkt(self.base + i, data[((self.start + self.base + i)*p):((self.start + self.base + i+1)*p)])
    ...
    def rdt_rec(self):
        while(self.base < self.end - self.start):
            try:
                ...
                print "SOCKET TIMEOUT, CLOSING THE SOCKET"           
                break
...
sock2.bind(("10.10.1.1", 0))
sock2.settimeout(3) # we use this to close the connection

for i in range(1):
    x = RDTSender(sock, "10.10.4.2", 9000, data, 0, 5000)
    y = RDTSender(sock2, "10.10.2.2", 9000, data, 5000, 10000)

    t1 = threading.Thread(target = x.run)
    t2 = threading.Thread(target = y.run)
