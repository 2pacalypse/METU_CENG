...
the routing table
"""
def tcp_listen(socket):
    while 1:
        connectionSocket, addr = socket.accept()
        message = connectionSocket.recv(1024)
        src, dst,_ , msg = parseMsg(message)
        #print "Recieved from"  + str(src) + " " +  "ultimate "  + str(dst)
        print msg
        if( links[node][routing_table[node][dst]] == 0):
            sendMsgUDP(message, routing_table[node][dst])
        else:
            sendMsgTCP(message, routing_table[node][dst])
...
"""
for each of the IP found above
open a socket on the current node
for listening
"""
...
"""
the thread library doesn't have
.join() so we have to loop infinitely.
The sockets are listening in the background always.
"""
while 1:
    pass   
